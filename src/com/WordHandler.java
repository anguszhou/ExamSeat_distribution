...
public class WordHandler {
	...
	private Dispatch document = null;// 代表进行处理的word 文档
	...
	// 段落的处理,插入格式化的文本
	 public void insertFormatStr(String text) { 
		 ...
		 Dispatch alignment = Dispatch.get(selection, "ParagraphFormat").toDispatch();// 段落格式    86.       
	...
	// word 中在对表格进行遍历的时候 ，是先列后行 先column 后cell 另外下标从1开始
	 public void insertTable(String tableTitle, int row, int column) {
		 ...
		 Dispatch.call(selection, "TypeParagraph"); // 空一行段落            
		 Dispatch.call(selection, "MoveDown"); // 游标往下一行   96.         
		 // 建立表格 
		 
		 Dispatch tables = Dispatch.get(document, "Tables").toDispatch(); 
		 Dispatch range = Dispatch.get(selection, "Range").toDispatch();// /当前光标位置或者选中的区域
		 Dispatch newTable = Dispatch.call(tables, "Add", range, 
				 new Variant(row), new Variant(column), new Variant(1)).toDispatch(); // 设置row,column,表格外框宽度
		 
		 Dispatch cols = Dispatch.get(newTable, "Columns").toDispatch(); // 此表的所有列，   107.         
		 int colCount = Dispatch.get(cols, "Count").changeType(                    
				 Variant.VariantInt).getInt();// 一共有多少列 实际上这个数==column   109.         
		 System.out.println(colCount + "列"); 
		 
		 for (int i = 1; i <= colCount; i++) { // 循环取出每一列    111.             
			 Dispatch col = Dispatch.call(cols, "Item", new Variant(i)).toDispatch();                 
			 Dispatch cells = Dispatch.get(col, "Cells").toDispatch();// 当前列中单元格   114.             
			 int cellCount = Dispatch.get(cells, "Count").changeType(                     
					 Variant.VariantInt).getInt();// 当前列中单元格数 实际上这个数等于row   116.             
			 for (int j = 1; j <= cellCount; j++) {// 每一列中的单元格数   117.                 
				 // Dispatch cell = Dispatch.call(cells, "Item", new   118.                 
				 // Variant(j)).toDispatch(); //当前单元格    119.                 
				 // Dispatch cell = Dispatch.call(newTable, "Cell", new    120.                 
				 // Variant(j) , new Variant(i) ).toDispatch(); //取单元格的另一种方法   121.                 
				 // Dispatch.call(cell, "Select");//选中当前单元格   122.                 
				 // Dispatch.put(selection, "Text",    123.                 
				 // "第"+j+"行，第"+i+"列");//往选中的区域中填值，也就是往当前单元格填值    124.                 
				 putTxtToCell(newTable, j, i, "第" + j + "行，第" + i + "列");// 与上面四句的作用相同   125.              
			 }
		 }
	 }
	 ...
	 public void putTxtToCell(Dispatch table, int cellRowIdx, int cellColIdx, String txt) {    
		 ...
		 Dispatch selection = Dispatch.get(MsWordApp, "Selection").toDispatch(); // 输入内容需要的对象   143.         
	...
	//在指定的单元格里填写数据
	 public void putTxtToCell(int tableIndex, int cellRowIdx, int cellColIdx,  String txt) {
		// 所有表格    157.         
		 Dispatch tables = Dispatch.get(document, "Tables").toDispatch();            // 要填充的表格    159.         
		 Dispatch table = Dispatch.call(tables, "Item", new Variant(tableIndex)).toDispatch();          
		 Dispatch cell = Dispatch.call(table, "Cell", new Variant(cellRowIdx),   new Variant(cellColIdx)).toDispatch();   
		 Dispatch.call(cell, "Select");   
		 Dispatch selection = Dispatch.get(MsWordApp, "Selection").toDispatch(); // 输入内容需要的对象   165.         
		 Dispatch.put(selection, "Text", txt);  
	 }
	// 合并两个单元格    168.     
	 ...
	 public void mergeCellTest() {
		 Dispatch tables = Dispatch.get(document, "Tables").toDispatch(); 
		 int tableCount = Dispatch.get(tables, "Count").changeType(Variant.VariantInt).getInt(); // document中的表格数量    182.         
		 Dispatch table = Dispatch.call(tables, "Item", new Variant(tableCount)).toDispatch();// 文档中最后一个table    184.         
		 mergeCell(table, 1, 1, 1, 2);// 将table 中x=1,y=1 与x=1,y=2的两个单元格合并   185.    
	 ...
	 public void moveUp(int pos) { 
		 Dispatch selection = Dispatch.get(MsWordApp, "Selection").toDispatch(); // 输入内容需要的对象          
		 for (int i = 0; i < pos; i++) {              // MoveDown MoveLeft moveRight              
	 ...
	 public boolean find(String toFindText) {          
		 ...
		 Dispatch selection = Dispatch.get(MsWordApp, "Selection").toDispatch(); // 输入内容需要的对象          
		 // 从selection所在位置开始查询          
		 Dispatch find = Dispatch.call(selection, "Find").toDispatch();          // 设置要查找的内容          
		 Dispatch.put(find, "Text", toFindText);          // 向前查找          
